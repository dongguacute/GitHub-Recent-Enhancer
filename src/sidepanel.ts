...
function renderList(items: GitHubItem[]): string {
  ...
  const rows = items.map(item => {
    const icon = item.pull_request ? OPEN_ICON : ISSUE_ICON;
  ...
  return `
    <div class="stats">
      <span class="stat stat-pr">${OPEN_ICON} ${prCount} PRs</span>
      <span class="stat stat-issue">${ISSUE_ICON} ${issueCount} Issues</span>
    </div>
